...
export function connectVkPlayChat({ channelId, token }, dispatch) {
        ...
        if (!channelId && !token) {
            dispatch(
                addNotice({
                    id: genRandStr(),
                    type: "error",
                    message: "Заполните поля ID канала и токена",
                }),
            );
        } else if (!token) {
            dispatch(
            ...
            dispatch(
                addNotice({
                    id: genRandStr(),
                    type: "error",
                    message: "Заполните поле ID канала",
                }),
            );
        }
        return null;
    ...
    ws.onmessage = (event) => {
        try {
            ...
            console.warn(
                "❌ Ошибка парсинга VK Play сообщения:",
                e,
                "Data:",
                event.data,
            );
    ...
    ws.onclose = (event) => {
        console.warn(
            `⚠️ Отключено. Code: ${event.code}, Reason: "${event.reason}"`,
        );
    ...
    ws.onerror = (err) => {
        ...
        dispatch(
            addNotice({
                id: genRandStr(),
                type: "error",
                message: "Ошибка соединения с VK Видео Live",
            }),
        );
    };
